<div class="container">
  <div class="row">
    <div class="col-sm-10 col-lg-6 mx-auto pt-3">
      <h1 class="py-md-3">Thanks your support!</h1>
      <div class="card mb-3">
	<img class="card-img-top" src="images/article/26804015_138111763527272_80447334_n.jpg" alt="Card image cap">
	<div class="card-body">
	  <a href="#" class="btn btn-primary btn-block">Download</a>
	</div>
      </div>
      <?php if(isset($productKeys) AND count($productKeys) > 1){?>
      <div class="card mb-3">
        <img class="card-img-top" src="images/article/26853157_138111770193938_1490175646_o.jpb" alt="Card image cap">
        <div class="card-body">
          <a href="#" class="btn btn-primary btn-block">Download</a>
        </div>
      </div>
      <?php }?>
      <a href="do_unset">home</a>
    </div>
  </div>
</div>
